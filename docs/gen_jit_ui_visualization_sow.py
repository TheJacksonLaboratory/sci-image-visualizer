...
labeled("Status", "Substantially delivered on the jit-ui 'openseadragon' branch; library extraction in progress, distribution pending")
...
para(
    ...
    "Channels and Histogram tool including 16-bit) is already implemented on the jit-ui 'openseadragon' "
    "branch and the supporting jit-service endpoints are deployed. The remaining work is finishing the "
...
para(
    ...
    "coupling and be published and reused (deliverables D6 and D7)."
)
table([
...
bullet("Migrating other JAX Data Science frontends onto the library (downstream adoption is named as a Planned deliverable, D7, but the actual per-consumer integration is owned by each consumer).")
...
para(
    ...
    "is implemented and verified on the jit-ui 'openseadragon' branch."
)
...
h2("D6. Extraction into the jax-image-visualization Nx buildable library")
...
table([
    ...
    ["Versioning / breaking changes destabilize downstream consumers",
     "Medium", "Medium",
     "Semantic versioning + a documented breaking-change policy as part of D7; keep the public surface small and contract-first."],
